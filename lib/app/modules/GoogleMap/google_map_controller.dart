...
class MapController extends GetxController {
  ...
  Future<void> fetchVendorLocations() async {
    ...
    try {
      ...
      var response = await http.post(
        Uri.parse('https://roadservice.roadservicerepair.com/api/show_map.php'),
        headers: {'Content-Type': 'application/json'},
        body: jsonEncode({"email_id": prefs.getString('email')}),
      );
  ...
  void _updateMarkers(List<dynamic> vendors) {
    ...
    for (var vendor in vendors) {
      double lat = double.parse(vendor['lat']);
      double lng = double.parse(vendor['lang']);
      LatLng position = LatLng(lat, lng);

      markers.add(Marker(
        markerId: MarkerId(vendor['vendor_email']),
        position: position,
      ...
      if (currentPosition != null) {
        fetchRoute(currentPosition!, position);
      }
    }
    update();
  }
